...
{
    public partial class EmployeeEditSearch : Form
    {
        BlEmployee _blEmployee;
        public EmployeeEditSearch()
        {
            InitializeComponent();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            if (rbTc.Checked)
            {
                ...
                empMenu.TcArama(txt1.Text);
                this.Close();
            }
            if (rbAdSoyad.Checked)
            {
                EmployeeEditMenu.EmployeeEditMenu empMenu = Application.OpenForms["EmployeeEditMenu"] as EmployeeEditMenu.EmployeeEditMenu;
                empMenu.AdSoyadArama(txt1.Text, txt2.Text);
                this.Close();
            }
            if (rbTel.Checked)
            {
                EmployeeEditMenu.EmployeeEditMenu empMenu = Application.OpenForms["EmployeeEditMenu"] as EmployeeEditMenu.EmployeeEditMenu;
                empMenu.TelArama(txt1.Text);
                this.Close();
            }
            if (rbMail.Checked)
            {
                EmployeeEditMenu.EmployeeEditMenu empMenu = Application.OpenForms["EmployeeEditMenu"] as EmployeeEditMenu.EmployeeEditMenu;
                empMenu.MailArama(txt1.Text);
                this.Close();
            }
        }
    }
